...
`part_number` is free text for now (no `part_id` FK) -- Phase 3's Parts
Inventory can add a nullable `part_id` column additively, with no backfill
pain since existing rows just get NULL.
"""
...
class LineItem(TimestampMixin, Base):
    ...
    __table_args__ = (Index("idx_line_items_entity", "entity_type", "entity_id"),)
