"""Invoice list view: shop-wide, searchable, filterable by status."""
...
_STATUS_FILTER_CHOICES = ["All", *[s.value for s in InvoiceStatus]]


class InvoiceListView(QWidget):
    invoice_selected = Signal(int)

    def __init__(self, viewmodel: InvoiceListViewModel) -> None:
        super().__init__()
        ...
        self._search_box.setPlaceholderText("Search by invoice #...")
        ...
        self.viewmodel.invoices_changed.connect(self._on_invoices_changed)
    ...
    def _on_invoices_changed(self) -> None:
        self._status_label.setText(f"{self.viewmodel.total} invoice(s)")
